<?php
    ...
    if(!isset($_SESSION['Username']) && $role!="cart"){
...
function renderForm ($quantity,$error)
{
	...
	while($row = mysql_fetch_array( $result )) {
		...
		echo '<td><a href="delete.php?cartID='. $row['cartID'].'"><button>Remove</button></a></td>';
		echo "</tr>";
	...
	if ($displayTotal !=0)
	{
		echo '<td><form action="checkOut.php" method="post"><input type="submit" name="checkOut" id="checkOut" value="Check Out"></form></td>';
		echo "</tr>"; 
	}
	else
	{
		echo '<td><form action="checkOut.php" method="post"><input type="submit" name="checkOut" id="checkOut" value="Check Out" disabled></form></td>';
		echo "</tr>"; 
	}
	// close table>
	echo "</table>";
